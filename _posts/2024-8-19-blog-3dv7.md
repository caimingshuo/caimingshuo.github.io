...
这里仍然是一堆参数配置的预备工作，这里对我比较新颖的是将这个训练的阶段写成了一个类`phase`,以及根据是否设置正则化`reg_interval`来进行不同的参数配置(正则化是自从`StyleGAN`系列中传出来的比较重要的一个概念，这个后面会**详细分析**),后面还有采用`torch.cuda.Event`来进行GPU运行时间的计算;下面还有个值得说道的是导出样本图片的设置：
...
首先是关于`kimg`这个参数，指的是每一千张图片，也即训练过程中是以千张作为训练单位，然后是判断`progress_fn`的存在，`progress_fn`和之前一起传入`training_loop`函数的`abort_fn`都是回调函数(~~又是我这个低级炼丹师没见过的高级操作~~),`abort_fn`用于决定是否终止训练过程，而`progress_fn`则是更新状态的函数，因此如果有这个功能(`progress_fn`存在话),会对根据当前训练的数据集进行一个参数的更新。代码接下来的思路就比较分明，首先是获取训练图片的方式：作者上来就是一个`with torch.autograd.profiler.record_function('data_fetch'):`给我一波下马威，这是使用了PyTorch 的性能分析工具`profiler`来标记和记录特定代码块的执行时间。
